...
export default function Navbar() {
    const { activeSection, setActiveSection, setScrollAnimation } =
    ...
    return (
        <div>
            ...
            <div
                className={
                    nav
                        ? `fixed top-0 left-0 z-[1000] h-screen w-full flex flex-col justify-center items-center ease-in-out duration-500 bg-black`
                        : 'fixed -top-[100%]'
                }
            >
                <div
                    className={`absolute top-0 left-0 flex justify-center items-center cursor-pointer ml-9 h-20`}
                    onClick={() => {
                        setNav(!nav);
                        toggle();
                    }}
            ...
            {/* desktop navbar */}
            <div
                className={`left-0 ${isDropdown ? 'top-0' : '-top-20'} z-[45] w-screen ${isShrunk ? 'h-16 bg-black bg-opacity-50 backdrop-blur-sm' : 'h-20'} ${isFixed ? 'fixed ease-in-out duration-300' : 'absolute'}`}
                ref={navbar}
            >
                {!isMobileView && (
                    <Link href="/#hero" tabIndex={-1}>
                        <div className="relative hover:opacity-85 ease-in-out duration-300">
                            <div
                                className={`absolute left-0 right-0 m-auto my-auto ${isShrunk ? 'w-16' : 'w-20'} mt-2 ease-in-out duration-300`}
                            >
                                <Image
                                    src="/misc/logo.webp"
                                    alt="logo"
                ...
                <div className="flex lg:hidden w-full justify-between h-full px-10">
                    <div
                        className="flex justify-center items-center -ml-1 cursor-pointer "
                        onClick={() => {
                            setNav(!nav);
                            toggle();
                        }}
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width={isShrunk ? 32 : 40}
                            height={isShrunk ? 32 : 40}
                            viewBox="0 0 32 32"
                        >
                            <path
                                fill="none"
                    ...
                    <div
                        className="flex justify-center items-center mr-4 relative cursor-pointer"
                        onClick={() => {
                            openCart();
                        }}
                    >
                ...
                <div className="hidden lg:flex flex-row w-screen h-full ">
                    <div className="w-1/2 h-full flex justify-end text-white">
                        <span
                            className={`mt-1 h-full  ${activeSection === 'about' ? 'border-opacity-100' : 'border-opacity-0'} hover:border-opacity-100 border-b-2 hover:opacity-85 border-custom-pink w-44 ease-in-out duration-300 out`}
                        >
                            <Link
                                href="/#about"
                                tabIndex={0}
                        ...
                        <span
                            className={`mt-1 h-full w-44 ${activeSection === 'tours' ? 'border-opacity-100' : 'border-opacity-0'} hover:border-opacity-100 hover:opacity-85 border-b-2 border-custom-pink ease-in-out duration-300`}
                        >
                            <Link
                                href="/#tours"
                                tabIndex={0}
                        ...
                        <span
                            className={`mt-1 h-full w-44 mr-24 ${activeSection === 'contact' ? 'border-opacity-100' : 'border-opacity-0'} hover:border-opacity-100 hover:opacity-85 border-b-2 border-custom-pink ease-in-out duration-300`}
                        >
                            <Link
                                href="/#contact"
                                tabIndex={0}
                    ...
                    <div className="w-1/2 right-1/2 h-full flex justify-between text-white">
                        <span
                            className={`mt-1 h-full w-44 ml-24 ${activeSection === 'store' ? 'border-opacity-100' : 'border-opacity-0'} hover:border-opacity-100 hover:opacity-85 border-b-2 border-custom-pink ease-in-out duration-300`}
                        >
                            <Link
                                href={'/store'}
                                tabIndex={0}
                        ...
                        <div
                            className="flex justify-center items-center w-44 relative cursor-pointer hover:opacity-85 ease-in-out duration-300"
                            tabIndex={0}
                            onClick={() => {
                                openCart();
                            }}
