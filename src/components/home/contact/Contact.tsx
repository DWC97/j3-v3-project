...
export default function Contact(): JSX.Element {
    ...
    return (
        <div
            className="bg-black w-full min-h-[972px] xl:pb-0 pb-28 xl:pt-20 pt-60 flex flex-col-reverse  gap-24 xl:gap-0 xl:flex-row items-center justify-center"
            id="contact"
            ref={contactRef}
        >
            {/* logo */}
            <div className="xl:mr-20 2xl:mr-28 flex flex-col items-center justify-center h-[500px]">
                ...
                <Reveal>
                    <Link
                        href={'/store'}
                        className="flex flex-row items-center justify-center mt-10 hover:opacity-85 ease-in-out duration-300"
                    >
                        <button
                            tabIndex={-1}
            ...
            {/* form */}
            <Reveal>
                <div className="flex flex-col px-10 sm:px-0 sm:w-[500px] md:w-[600px] h-[600px] sm:h-[500px]">
                    <h1 className="font-semibold text-[32px] md:text-[44px] text-white mb-4">
                        RESERVE A SPOT
                    </h1>
                    <form
                        onSubmit={handleSubmit}
                        className="flex flex-col justify-around h-[600px] sm:h-[500px]"
                    >
                        <div className="flex flex-col relative w-full sm:w-[220px] md:w-[260px]">
                            ...
                            <div
                                className={`${formErrors.name ? 'opacity-100' : 'opacity-0'} ease-in-out duration-300 transition-opacity absolute right-0 top-5 z-10`}
                            ...
                            <span
                                className={`${formErrors.name ? 'opacity-100' : 'opacity-0'} ease-in-out duration-300 transition-opacity absolute text-red-600 text-[12px] left-[0.75rem] top-[3.75rem]`}
                            >
                                {formErrors.name}
                            </span>
                        </div>
                        <div className="flex flex-col relative w-full sm:w-[220px] md:w-[260px]">
                            ...
                            />
                            <div
                                className={`${formErrors.email ? 'opacity-100' : 'opacity-0'} ease-in-out duration-300 transition-opacity absolute right-0 top-5 z-10`}
                            ...
                            <span
                                className={`${formErrors.email ? 'opacity-100' : 'opacity-0'} ease-in-out duration-300 transition-opacity absolute text-red-600 text-[12px] left-[0.75rem] top-[3.75rem]`}
                            >
                                {formErrors.email}
                            </span>
                        </div>
                        ...
                        <button
                            type="submit"
                            className={`${submittable ? 'hover:opacity-85 ease-in-out duration-300' : ''} ${isSubmitting ? 'opacity-85 hover:opacity-85' : 'opacity-100'} font-semibold flex justify-center items-center  w-full self-center rounded-md  bg-gradient-to-r from-custom-orange to-custom-pink p-[2px] my-2`}
                            title={
                                submittable
                                    ? ''
                                    : 'Please fill out each field in the form correctly'
                            }
                        >
                            <div
                                className={`w-full h-full rounded-md py-[6px] ${submittable ? 'bg-transparent' : 'bg-black'}`}
                            >
                                <div
                                    className={`w-full flex justify-center items-center ${submittable ? 'text-white' : 'bg-gradient-to-r from-custom-orange to-custom-pink text-transparent bg-clip-text'}`}
                                >
                        ...
                        <p className="text-white text-[12px] sm:text-[14px]">
                            Please note. Jolly Roger Tours is currently going
                            through the incorporation and licensing process in
                            the UK and will launch officially in late 2024.
                        </p>
                    </form>
                </div>
            </Reveal>
